...
#Step1 - Read features.txt - for replacing measurements default column names 
setwd(file.path("~",directory))
featureNames = read.table("features.txt")

#Step2 - Read activity labels - for replacing Activities with actual names
setwd(file.path("~",directory))
activityNames = read.table("activity_labels.txt")


#Step3 - Read Test Data
...
#Step5 - rbind both the data
#this is part 1 "Merges the training and the test sets to create one data set."
finalData = rbind(finalTestData,finalTrainData)


#Step6 - this is for part 2 "Extracts only the measurements on the mean and standard deviation for each measurement."
...
#Step7 - this is for part 3 "Uses descriptive activity names to name the activities in the data set"
for(i in 1:6) {
        finalObsData$Activity[finalObsData$Activity == i] <- as.character(activityNames$V2[activityNames$V1 == i])
}

#Step8 - this is part 4 "Appropriately labels the data set with descriptive variable names."
names(finalObsData) <- columnFeatureNames


#Step9 - this is for part 5 "From the data set in step 4, creates a second, independent tidy data set with the average of each variable"
...
#Step10 - Write tidy dataset to an output text file
